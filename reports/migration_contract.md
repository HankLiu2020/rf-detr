...
> Status: **DRAFT / NOT FROZEN**. Round 2 correctness is implemented, but numerical policy choices and effect claims require real-data E0–E5 validation.
...
7. **Probe 指标**：固定 train split、resize、eval/no-grad/sequential，先按可配置 score threshold 过滤，再做一对一 IoU matching；unmatched GT=FN、unmatched prediction=FP、matched wrong class=class error，三者不重复计数，并恢复 model train state。
...
12. **推荐 Weight 范围**：初始 state weights `MASTERED=.8, LEARNING=1.0, HARD_LEARNABLE=1.3, SUSPECT=.8`，默认 clip `[.7, 1.3]`。classification 做 image-mean normalization；box/mask/keypoint 做 global matched-instance-mass normalization，不能把 image mean=1 解释成所有 branch 的有效 LR 自动不变。
...
14. **DDP 必要条件**：各 rank observations gather 到 rank0，rank0 生成唯一 StateStore/policy snapshot 并 broadcast；Sampler global plan 同样由 rank0 broadcast。所有 rank 仍需相同 dataset stable-ID 顺序、quota、epoch 和每-rank `num_samples`，且不能被 Lightning 第二个 sampler 覆盖。
